...
static void SDL_RenderArc(
	...
	const float thicknessStep = 0.1f; //arbitrary value to avoid overlapping lines. Changing THICKNESS_STEP or numCircleSegments might cause artifacts.
...
Clay_Dimensions SDL_Clay_MeasureText(
	...
	/*ReSharper disable once CppParameterNeverUsed*/ uintptr_t userData
) {
	int width = 0;
	...
	for (uint32_t i = 0; i < rcommands->length; i++) {
		...
		switch (rcmd->commandType) {
			...
			case CLAY_RENDER_COMMAND_TYPE_TEXT: {
				...
				SDL_Surface* surface = TTF_RenderText_Blended(font, config->stringContents.chars,
			...
			case CLAY_RENDER_COMMAND_TYPE_BORDER: {
				...
				//edges
				SDL_SetRenderDrawColor(renderer, (int) config->color.r, (int) config->color.g, (int) config->color.b,
				                       (int) config->color.a);
				...
				if (config->width.bottom > 0) {
					...
					SDL_SetRenderDrawColor(renderer, (int) config->color.r, (int) config->color.g,
					                       (int) config->color.b,
					                       (int) config->color.a);
					SDL_RenderFillRect(renderer, &line);
				}
